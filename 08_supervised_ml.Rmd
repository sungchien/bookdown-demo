...
### 複雜字串資料類型
IMDB 5000 Movie Dataset中，為了可以表示多個可能的值，`genres`(電影類型)、`keywords`(關鍵詞)和`production_companies`製作公司都是屬於較複雜的字串資料類型，而且都是以JSON的方式表示這些值。例如
...
依據同樣方式分析電影的keywords，找出共有多少個keywords，keywords的電影數量分布範圍為何
```{r}
movies$keywords_data <- sapply(movies$keywords, valueExtract)
```

將電影的每一個keywords展開
```{r}
movies_keywords <- movies %>%
  select(id, keywords_data) %>%
  mutate(keywords_data=strsplit(keywords_data, split="%")) %>%
  unnest(keywords_data) %>%
  mutate(value=1)
```

統計每一個keywords出現的電影數，按照大小排列
```{r}
movies_keywords %>%
  group_by(keywords_data) %>%
  summarise(count=sum(value)) %>%
  arrange(desc(count))
```
...
取出包含前二十個關鍵詞的電影，並且轉成wide format
```{r}
movies_keywords <- movies_keywords %>%
  filter(keywords_data %in% top_keywords) %>%
  mutate(keywords_data=paste0("k_", keywords_data)) %>%
  spread(key=keywords_data, value=value, fill=0)
```

合併兩個data frame，捨棄keywords和keywords_data兩個variables，並且將keywords相關variables的`NA`值改為0。
```{r}
na2zero <- function (x) {
  ifelse(is.na(x), 0, x)
}
...
movies <- movies %>%
  left_join(movies_keywords) %>%
  select(-keywords, -keywords_data) %>%
  mutate_at(vars(matches("k_")), na2zero)
```
...
set.seed(sd)
permuted_id <- sample(movies$id, nrow(movies))

folds <- 5
test_iter <- 1

test_set <- movies %>%
  filter(id %in% permuted_id[seq(test_iter, nrow(movies), folds)])
