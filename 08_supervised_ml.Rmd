...
### 複雜字串資料類型
IMDB 5000 Movie Dataset中，為了可以表示多個可能的值，`genres`(電影類型)、`keywords`(關鍵詞)和`production_companies`製作公司都是屬於較複雜的字串資料類型，而且都是以JSON的方式表示這些值。因此，為了表示這些值，首先載入JSON的處理套件`jsonlite`。
