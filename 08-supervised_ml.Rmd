...
classifier_comp <- classifier_comp %>%
  add_row(method="Decision Tree", result=acc)
```
...
隨機森林(random forest)是一個集成式學習(ensemble learning)的分類方法，也就是是將多個分類結果整合起來的方法。隨機森林的得名來自於它是由多個決策樹的分類結果整合起來的方法，並且每一個決策樹的訓練資料集是從原本的訓練資料集**隨機**選取若干observations和variables進行訓練。
