...
- 能夠安裝R 語言的開發環境，包括核心程式以及整合開發環境(Integrated Development Environment, IDE)
- 能夠運用IDE 的Console 撰寫R 語言的指令
...
- CRAN：下載R語言的執行檔、原始碼和說明文件，也收錄了各種套件
...
請在Console下輸入下面的敘述，分別查看c, d, ee，說明為何c, d, e 的數值為何不同？
...
如果要找出vector中的元素在由大到小順序中的位置，則可以在函數`order()`中，將`decreasing`參數設為TRUE。
