...
<!--chapter:end:04-data_visualization.Rmd-->
...
<!--chapter:end:05-exploratory_data_analysis.Rmd-->
...
<!--chapter:end:06-statistical_analysis.Rmd-->
...
<!--chapter:end:07_unsuperivsed_ml.Rmd-->
...
預測的目標variable為`vote_average`，檢視這個variable的分布情形。
```{r}
movies %>%
  ggplot() +
...
根據`vote_average`的分布情形，決定將這個variable分為x<=5, 5<x<=6, 6<x<=7以及7<x等四個區間，將`vote_average`改為新的變數`vote_average_p`，做為接下來預測的目標。也就是以其他的variables來預測`vote_average_p`在哪一個區間(分類)。
...
以訓練好的分類樹模型進行訓練資料(train_set)分類的成效
```{r}
...
由於模型是由訓練資料內的模式產生，一般來說，訓練資料的分類成效會比測試資料來得好。

### 利用caret套件的rpart方法進行分類樹監督式學習
...
以訓練好的分類模型進行訓練資料(train_set)分類的成效
```{r}
...
<!--chapter:end:08_supervised_ml.Rmd-->
