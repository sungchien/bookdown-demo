...
資料探勘的過程中運用機器學習的技術，可以依據是否有一部份資料中加入人類判斷、用來做為預測其他資料的訊息大致區分成**監督式學習(supervised learning)**和**非監督式學習(unsupervised learning)**。以上述的應用而言，預測大多屬於監督式學習，而集群則一般認為是非監督式學習。本章介紹非監督式學習中的集群分析。
