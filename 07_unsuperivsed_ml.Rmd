...
**資料探勘 (Data Mining)**是利用電腦演算法發掘大量資料中蘊藏的規律性、樣式或知識，以幫助人們了解與說明資料的特性或者應用規律性與樣式來預測新的資料，而通常來說這些規律性與樣式以人的認知能力並不容易發現。一般來說，資料探勘的任務目的包括預測(prediction)、關連(association)、群集(clustering)、異常偵測(anomaly detection)等等。
...
- 群集是根據Observations之間的相似性(或者距離)，找出劃分Observations成若干群的方式，盡可能使得群內的Observations彼此間的相似性大於任何群外的Observations (或是彼此間的距離小於群外的距離)。相關的例子有根據詞語的上下文資訊找出語意相關的詞語群集。 (舉例而言，跟財稅金融有關的詞語它們的上下文資訊應當很相像。未來會有一章專門介紹詞語的群集。)
...
資料探勘的過程中運用機器學習的技術，可以依據一部份資料中是否加入人類判斷的訊息，用來做為預測其他資料的
...
```{r}
tsne_out <- Rtsne(sta_d, perplexity=5, theta=0.0, is_distance=TRUE)
```
