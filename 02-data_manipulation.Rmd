...
```{r echo=FALSE}
knitr::include_graphics("image/epa_opendata.png")
```

2. 將上面的網頁往下拉到大氣類的空氣品質指標

```{r echo=FALSE}
knitr::include_graphics("image/epa_aqi_search.png")
```

3. 進入[空氣品質指標(AQI)](https://opendata.epa.gov.tw/Data/Contents/AQI/)

```{r echo=FALSE}
knitr::include_graphics("image/epa_aqi_data.png")
```
...
```{r echo=FALSE}
knitr::include_graphics("image/aqi_df.png")
```
...
```{r echo=FALSE}
knitr::include_graphics("image/save_encoding.png")
```

未來有需要再開啟這個Script 的話，可以利用File 下的**Reopen with Encoding**，再選擇UTF-8，重新開啟。

```{r echo=FALSE}
knitr::include_graphics("image/reopen_encoding.png")
```
